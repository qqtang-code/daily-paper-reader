...
score: 7.0
evidence: 音频-视觉事件识别的交叉注意力压缩
tldr: "针对音频-视觉事件识别中Transformer模型计算复杂、部署困难的问题，提出结合架构压缩、知识蒸馏与动态INT8量化的高效压缩框架。教师模型采用VideoMAE和AST提取特征，经混合交叉注意力融合；轻量学生模型通过缩减隐藏维度、注意力头数和前馈网络构建，并从教师蒸馏学习。在AVE数据集上，融合模块参数量减少59.06%，准确率仅降2.14%，模型大小从10.71MB压缩至2.04MB。该方法在资源受限设备上实现了准确率与效率的良好权衡。"
source: arxiv
...
motivation: 降低Transformer多模态模型在边缘设备上的部署成本，解决计算复杂度高和内存占用大的问题。
method: 通过缩减隐藏维度、注意力头数和前馈网络构建轻量学生模型，结合知识蒸馏与动态INT8后训练量化实现压缩。
result: "融合模块参数减少59.06%，分类准确率仅下降2.14%，模型大小从10.71MB降至2.04MB。"
conclusion: 框架在保持高识别性能的同时显著降低计算开销，适用于资源受限的边缘AI平台。
---

## 摘要
音视频事件识别（AVER）通过基于transformer的多模态架构取得了显著的性能提升。然而，这些模型的高计算复杂度、大内存占用和推理成本阻碍了它们在边缘设备和资源受限设备上的部署。本文提出了一种高效的压缩框架，用于基于混合交叉注意力的音视频事件识别，结合了架构模型压缩、知识蒸馏和动态INT8量化。一个高容量的教师模型集成了用于视觉表征学习的VideoMAE、用于音频特征提取的音频频谱图Transformer（AST）以及用于多模态特征融合的混合交叉注意力融合网络。通过降低隐藏特征维度、注意力头数和前馈网络大小，同时保留整体网络架构，构建了一个轻量级的学生模型。学生模型使用知识蒸馏进行训练，以有效地从教师模型中迁移判别性知识。最后，应用动态INT8训练后量化进一步减小模型大小以实现高效部署。在音视频事件（AVE）数据集上的实验结果表明，所提出的框架将多模态融合模块的可训练参数减少了59.06%，与教师模型相比，分类准确性仅下降了2.14%。此外，动态INT8量化将模型大小从10.71 MB减小到2.04 MB，同时保持了有竞争力的识别性能。这些结果表明，所提出的框架在识别准确性和计算效率之间提供了有效的权衡，使其成为资源受限的边缘AI平台上有前途的部署解决方案。
